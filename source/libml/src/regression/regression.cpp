...
class Regression::Imple
{
 public:
  ...
  setDimensions(TimeSeriesMap const& time_series_map)
  {
    ...
    if (_n_dim_D < 2 || time_series_map.size() == 0)
      throw ex_bix;
    ...
    for (auto const& it : time_series_map)
    {
      ...
      if (it.first < 0 || it.first > _n_dim_D - 1)
        throw ex_or;
      // each sample data dimension should be all the same.
      if (_n_dim_Y != static_cast<int>(it.second.size()))
        throw ex_bix;
    }
  }
};

Regression::Regression(int const& n_dim_D)
...
void
Regression::setDimensions(TimeSeriesMap const& time_series_map)
{
  _p->setDimensions(time_series_map);
}

int const&
Regression::timeDimension() const
{
  return _p->_n_dim_D;
}

int const&
Regression::xDimension() const
{
  return _p->_n_dim_X;
}

int const&
Regression::yDimension() const
{
  return _p->_n_dim_Y;
}

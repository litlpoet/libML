...
class MultiLevelBSplineInterpolation::Imple
{
 public:
  ...
  bool solveBSpline(int const& D, int const& D_X, int const& n_knots, MatNxN* M)
  {
    MatNxN CPS;
 ...
 private:
  ...
  void solveControlPoints(int const& D, int const& D_X, int const& n_knots, MatNxN* CPS)
  {
  ...
  void interpolate(int const& D, int const& D_X, int const& n_knots, MatNxN const& CPS, MatNxN* M)
  {
  ...
  int localSplineDomain(int const& D, int const& n_knots, int const& f, float* t_l)
  {
  ...
  RVecN evaluate(int const& D, int const& D_X, int const& n_knots, int const& f, MatNxN const& CPS)
  {
    float t_l;
  ...
  RVecN evaluateLocal(float const& t_l, Eigen::MatrixBase<Derived> const& local_pts)
  {
    return computeBasis(t_l) * local_pts;
  }
};

MultiLevelBSplineInterpolation::MultiLevelBSplineInterpolation(int const&           D,
                                                               TimeSeriesMap const& time_series_map)
    : Interpolation(D, time_series_map), _p(new Imple(D, dataDimension(), time_series_map))
{
}
...
bool
MultiLevelBSplineInterpolation::solve(int const& initial_n_knots,
                                      int const& level,
                                      MatNxN*    result_mat)
{

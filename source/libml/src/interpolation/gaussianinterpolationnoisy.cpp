...
class GaussianInterpolationNoisy::Imple
{
 public:
  ...
  Imple(int const& n_dim_D, int const& n_dim_X, TimeSeriesMap const& time_series_map)
      : _n_X(time_series_map.size())
  {
    prepareSystem(n_dim_D, n_dim_X, time_series_map);
  }

  Imple(int const&             n_dim_D,
        int const&             n_dim_X,
  ...
  bool solveSigmaAndMu(int const&   n_dim_D,
                       int const&   n_dim_X,
                       float const& lambda,
                       float const& alpha,
                       MatNxN*      Mu,
                       MatNxN*      Sig_x_y)
  {
    ...
    for (int i   = 0; i < n_dim_X; ++i)
      Mu->col(i) = LGS * _Y->col(i);
    return true;
  }

 private:
  void prepareSystem(int const& n_dim_D, int const& n_dim_X, TimeSeriesMap const& time_series_map)
  {
    int i = 0;
  ...
  void prepareSystem(int const&             n_dim_D,
                     int const&             n_dim_X,
                     int const&             sampling_rate,
                     TimeSeriesDense const& time_series_dense)
  {
...
GaussianInterpolationNoisy::GaussianInterpolationNoisy(int const&           n_dim_D,
...
bool
GaussianInterpolationNoisy::solve(float const& lambda,
                                  float const& alpha,
                                  MatNxN*      Mu,
                                  MatNxN*      Sig_x_y)
{
...
void
GaussianInterpolationNoisy::setBoundaryConstraint(int const& b_type)
{
